...
router.get('/', async (req, res) => {
    try {
        ...
        const tickets = await db_1.db.all(sql, params);
        res.json(tickets);
    }
    catch (error) {
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const ticket = await db_1.db.get('SELECT * FROM tickets WHERE id = ?', [id]);
        if (!ticket) {
...
router.post('/', async (req, res) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `;
        const result = await db_1.db.run(sql, [
            title,
            description || '',
            status || 'todo',
            priority || 'medium',
        ...
        const newTicket = await db_1.db.get('SELECT * FROM tickets WHERE id = ?', [result.lastID]);
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { title, description, status, priority, tags, assignee } = req.body;
        const existingTicket = await db_1.db.get('SELECT * FROM tickets WHERE id = ?', [id]);
        if (!existingTicket) {
            res.status(404).json({ error: 'Ticket not found' });
            return;
        }
    ...
    `;
        ...
        const updatedTicket = await db_1.db.get('SELECT * FROM tickets WHERE id = ?', [id]);
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        await db_1.db.run('UPDATE tickets SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [status, id]);
        const updatedTicket = await db_1.db.get('SELECT * FROM tickets WHERE id = ?', [id]);
        res.json(updatedTicket);
    }
    catch (error) {
